...
struct LocalLockNode_ {
  ...
  LocalLockNode_() : read_current(0), read_ticket(0), read_handover(0), write_current(0), write_ticket(0), write_handover(0),
...
inline uint64_t get_hashed_local_lock_index(const GlobalAddress& addr) {
  ...
  return addr.raw % define::kLocalLockNum;
}
...
private:
  LocalLockNode_ local_locks[define::kLocalLockNum];
};
